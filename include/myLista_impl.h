...
#ifndef MYLISTA_IMPL_H
#define MYLISTA_IMPL_H
...
*/
template < typename T>
typename myLista<T>::iterator myLista<T>::before_end()
{
	typename myLista<T>::iterator cit(sentinela_tail);	// NULL = sentinela_tail->proximo
	return cit;
}
...
*/
template < typename T>
typename myLista<T>::iterator myLista<T>::erase( myLista<T>::iterator &it )
{
	...
	it++;	// avança iterator passado por referência;

	myLista<T>::iterator _retorno = it;	// atribui o iterator avançado para um novo iterator;
...
template < typename T>
typename myLista<T>::iterator myLista<T>::insert( iterator &it, T elem )
{

	myLista<T>::iterator _retorno = it;	// Cria iterator a partir do passado

	if( empty() or it == begin() )	// Se o it apontar para o início da lista ou a lista estiver vazia
	{
		push_front(elem);
		return begin();
	}

	_retorno--;	// Recua o iterator criado

	if( it == end() )	// Se o it apontar para o início da lista ou a lista estiver vazia
	{
		push_back(elem);
		return _retorno;
	}
	...
	new_node->proximo = it.atual;
	new_node->anterior = it.atual->anterior;

	it.atual->anterior->proximo = new_node;
	
	it.atual->anterior = new_node;
